...
const Members: React.FC = () => {
  ...
  const fetchMembers = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://backend-sisteminformasi-production.up.railway.app/users', {
        headers: {
  ...
  const handleDeleteMember = async (memberId: string) => {
    ...
    try {
      const response = await fetch(`https://backend-sisteminformasi-production.up.railway.app/users/${memberId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isFormMode === 'create') {
        ...
        const response = await fetch(`https://backend-sisteminformasi-production.up.railway.app/users/${selectedMember.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
